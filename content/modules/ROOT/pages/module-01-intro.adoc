...
+
파란색 배지를 가진 템플릿은 **_Source available_**을 나타내며, 자동으로 다운로드되어 저장된 템플릿 디스크를 사용합니다.
+
...
. 해당 Pod의 상세 페이지가 나타납니다. Node라는 제목의 섹션에서 파드가 실행 중인 워커 노드 이름을 확인할 수 있습니다. 아래 스크린샷에서는 **_worker-cluster-ttgmt-3_**에서 실행되고 있습니다.
+
